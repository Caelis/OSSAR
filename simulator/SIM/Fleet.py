...
def create_aircraft(ATC_list,r,t,dt): #creates aircraft when nessecary
    for i in xrange(len(ar_database)):               # add aircraft if needed
        if int(ar_database[i][-1]) > (t - 0.5*dt) and int(ar_database[i][-1]) < (t + 0.5*dt):
            ...
            new_plane = aircraft(plane_type,speed,max_acc,max_dcc,x1,y1,x1,y1,x2,y2,heading,ATC_gate,ATC_runway)
