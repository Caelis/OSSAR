...
def aircraft_interval(t_next_aircraft,idnumber,ATC_list,aircraft_list,runway_list,r,v_max,create,mean,std,originalGraph,currentGraph,decentralizedGraph,t,dt):
    if create == True:
        idnumber = create_aircraft(idnumber,ATC_list,aircraft_list,runway_list,r,v_max,originalGraph,currentGraph,decentralizedGraph,t,dt)
...
def create_aircraft(idnumber,ATC_list,aircraft_list,runway_list,r,v_max,originalGraph,currentGraph,decentralizedGraph,t,dt): #creates aircraft when nessecary
    ...
    ATC_list[ATC_gate].plan_operation(new_plane,currentGraph,t)
...
#loops through all ATC and appends a
def update_all_ATC_decentralized(ATC_list,runway_list,linkAtcRelations,currentGraph,radar_range,runway_occupance_time,dt,t,v_max):
    ## each atc
    for atc in ATC_list:
        ### radar (distance of aircraft from ATC)
        ### handoff decision
        ### graph update
        ### heading and speed commands
        currentGraph = atc.update(ATC_list,runway_list,linkAtcRelations,currentGraph,v_max,runway_occupance_time,dt,t) # check if commands for the plane are necessary and plan operation
        # aircraft_radar_list(atc)    # check for each aircraft which other plane are within a certain(radar) range
    return currentGraph
