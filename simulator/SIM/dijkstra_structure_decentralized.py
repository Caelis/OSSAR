...
def update_dijsktra(ATC_list, linkAtcRelations, graph, seperation, v_max):  # this function updates the graph values for the  current situation
    for key, value in graph.adjacency_iter():
        for inner_key, inner_value in value.items():
            ...
            if speed > 0:
                ...
                weight = float('Inf')
            graph[key][inner_key]['weight'] = weight

            for atc in linkAtcRelations[key][inner_key]:
                atc.graph[key][inner_key]['weight'] = weight
    return graph
